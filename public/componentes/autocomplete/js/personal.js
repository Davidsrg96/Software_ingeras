function autocomplete(inp, out, out2, arr, tipo) {
			...
			if(tipo == "rut"){
				if (arr[i].rut.substr(0, val.length).toUpperCase() == val.toUpperCase()) {
					...
					b.innerHTML += "<input type='hidden' id='" + arr[i].id + 
										"' name='" + arr[i].nombre + '/-/' + arr[i].rut  +"'>";
					/*execute a function when someone clicks on the item value (DIV element):*/
					b.addEventListener("click", function(e) {
						/*insert the value for the autocomplete text field:*/
						inp.value = this.getElementsByTagName("input")[0].name.split('/-/')[1];
						out.value = this.getElementsByTagName("input")[0].name.split('/-/')[0];
